...
const getConversation = asyncHandler(async (req, res) => {
  ...
  const conversationInfo = await checkIfInConvo(
    req.user.id,
...
const addMessageToConversation = asyncHandler(async (req, res) => {
  ...
  if (formData.conversationid) {
    const checkConvo = await checkIfInConvo(
      req.user.id,
      formData.conversationid,
    );
    if (!checkConvo) {
    ...
    await createMessage(req.user.id, new Date(), {
      convoid: formData.conversationid,
  ...
  const diferentCheckConvo = await checkIfConvoExistsByUsers(
    req.user.id,
    formData.targetid,
  );
...
const createConvo = asyncHandler(async (req, res) => {
  ...
  const checkConvo = await checkIfConvoExistsByUsers(
    req.user.id,
    formData.targetid,
  );
